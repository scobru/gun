...
function watch(what, options) {
  ...
  options = options ?? { msg: true, hubignore: false, alias: username }
  
  options.msg = options.msg ?? true;
  options.hubignore = options.hubignore ?? false;
  options.alias = options.alias ?? username
    ...
    watcher
      .on('add', async function(path) { 
        ...
        if (options.hubignore && path.includes('.hubignore')) {
          ...
          hubignore = await fs.promises.readFile(what + '/.hubignore', 'utf-8');
          ...
          if(path[path.search(/^./gm)] === "/" || ".") {
            gun.get('hub').get(modifiedPath + path.split(username)[1]).put(content)
          } else {
            gun.get('hub').get(modifiedPath + '/' + path.split(username)[1]).put(content)
          }
      ...
      .on('unlink', async function (path) {
        ...
        if (options.hubignore && path.includes('.hubignore')) {
          ...
          if(path[path.search(/^./gm)] === "/" || ".") {
            gun.get('hub').get(modifiedPath + path.split(username)[1]).put(null)
          } else {
						gun.get('hub').get(modifiedPath + '/' + path.split(username)[1]).put(null)
          }
