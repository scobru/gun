; (function () {
  ...
  ; (function () {
    if (screen.width > screen.height) { return } // phone only debug
  ...
  kit.views = new Map;
  (kit.watch = new MutationObserver(function (eve, b, low) {
    eve.forEach(function (changes) {
      changes.addedNodes.forEach(function (node) { //console.log("observed change on", node);
  ...
  kit.fs = {
    ...
    createReadStream(url) {
      url = (url || '').replace(location.__dirname + '/', '').split('#')[0];
  ...
  W[ON]('DOMContentLoaded', function (m) {
    ...
    (function () {
      function change(eve) {
        eve = eve || ''; eve = eve.detail || eve.data || eve;
